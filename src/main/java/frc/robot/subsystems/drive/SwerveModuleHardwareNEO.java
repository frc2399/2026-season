...
public class SwerveModuleHardwareNEO implements SwerveModuleIO {
    ...
    private static final AngularVelocity DRIVE_WHEEL_FREE_SPEED =
            RotationsPerSecond.of(
                    (MotorConstants.NEO_FREE_SPEED.in(RotationsPerSecond)
                                    * WHEEL_CIRCUMFERENCE.in(Meters))
                            / (DRIVING_MOTOR_REDUCTION));

    private static final Distance DRIVING_ENCODER_POSITION_FACTOR =
            (WHEEL_DIAMETER.times(Math.PI))
                    .div(DRIVING_MOTOR_REDUCTION)
                    .div((260.0 / 254)); // meters
    ...
    public SwerveModuleHardwareNEO(
        ...
        sparkMaxClosedLoopConfigDriving
