...
public class ShooterIndexerHardwareBeta implements ShooterIndexerIO {
    ...
    private static final AngularVelocity SHOOTER_INDEXER_VELOCITY_CONVERSION_FACTOR = RadiansPerSecond
            .of(2 * Math.PI / 60);
    ...
    private static final double SHOOTER_INDEXER_KS = 0;
    private static final double SHOOTER_INDEXER_KV = 0;
    ...
    public ShooterIndexerHardwareBeta() {
        ...
        shooterIndexerSparkFlexConfig.inverted(SHOOTER_INDEXER_MOTOR_INVERTED).idleMode(SHOOTER_INDEXER_IDLE_MODE)
                .smartCurrentLimit((int) MotorConstants.VORTEX_CURRENT_LIMIT.in(Amps));
        shooterIndexerSparkFlexConfig.encoder
                .positionConversionFactor(SHOOTER_INDEXER_POSITION_CONVERSION_FACTOR.in(Radians))
                .velocityConversionFactor(SHOOTER_INDEXER_VELOCITY_CONVERSION_FACTOR.in(RadiansPerSecond));
        shooterIndexerSparkFlexConfig.closedLoop.feedbackSensor(FeedbackSensor.kPrimaryEncoder)
        ...
        shooterIndexerClosedLoopConfig.feedForward.sva(SHOOTER_INDEXER_KS, SHOOTER_INDEXER_KV, SHOOTER_INDEXER_KA);

        shooterIndexerSparkFlexConfig.apply(shooterIndexerClosedLoopConfig);

        shooterIndexerSparkFlex = new SparkFlex(MotorIdConstants.SHOOTER_INDEXER_BETA_CAN_ID, MotorType.kBrushless);
        shooterIndexerSparkFlex.configure(shooterIndexerSparkFlexConfig, ResetMode.kResetSafeParameters,
    ...
    public void updateStates(ShooterIndexerIOState state) {
        ...
        state.shooterIndexerAppliedVoltage = shooterIndexerSparkFlex.getBusVoltage()
