...
public class RobotContainer {
        ...
        private ShooterIndexerSubsystem shooterIndexerSubsystem = subsystemFactory.buildShooterIndexer();

        // this is public because we need to run the visionPoseEstimator periodic from
        // Robot
        public VisionPoseEstimator visionPoseEstimator = new VisionPoseEstimator(drive,
                        subsystemFactory.getRobotType());
        public CommandFactory commandFactory = new CommandFactory(
                        ...
                        spindexerSubsystem,
                        intakeSubsystem);
        public AutonCommandFactory autonCommandFactory = new AutonCommandFactory(drive, intakeSubsystem, commandFactory,
                        gyro);
        ...
        private static final CommandXboxController tuningController = new CommandXboxController(
                        DriveControlConstants.TUNING_CONTROLLER_PORT);

        private final Alert driverDisconnected = new Alert("Driver controller disconnected!", AlertType.kWarning);
        private final Alert noAutonSelectedAlert = new Alert("Auton is not selected!", AlertType.kWarning);
        private final Alert lowBatteryAlert = new Alert(
                        ...
                        AlertType.kWarning);
        private final Alert gyroNeedsConfiguringAlert = new Alert(
                        "Gyro not configured! Auton + vision will not work; click the button below auton selector.",
                        AlertType.kWarning);
        private final Alert autonHasNoGyroAngleAlert = new Alert("Selected auton will not configure gyro properly!",
                        AlertType.kWarning);
        ...
        private void configureButtonBindingsDriver() {
                Trigger isAutoAligned = new Trigger(
                                () -> RebuiltVisionUtil.isShootingAngleAlignedToHub(() -> drive.getPose()));
                ...
                driverController
                                .leftTrigger().and(isAutoAligned)
                                .whileTrue(commandFactory.runSpindexShooterIndexAndShooter(false)); // do not pass
                driverController
                                .leftTrigger().and(isAutoAligned.negate())
                                .whileTrue(drive.autoAlignToTarget());
                driverController
                                .rightBumper()
        ...
        private void configureButtonBindingsTuningController() {
                ...
                tuningController.b().and(tuningController.leftBumper()).onTrue(Commands.print("hello!"));

                tuningController
                                .a()
                                .whileTrue(
                                                commandFactory
                                                                .shooterSpeedTuningRunSpindexerShooterIndexerShooterAndFeedFuel());
                tuningController
                                .povUp()
                                .onTrue(
                                                Commands.runOnce(
        ...
        public void setAlerts() {
                lowBatteryAlert.set(
                                (RobotController.getBatteryVoltage() > 0.0
                                                && RobotController
                                                                .getBatteryVoltage() <= DriveControlConstants.LOW_BATTERY_VOLTAGE));
